...
class Module {
  ...
  constructor(title, course_num, instructor, time, total_slots, description) {
    this.title = title;
  ...
  toString() {
    return this.course_num + ": " + this.title;
  }
}
...
const runAlgorithm = () => {
  ...
  var students = [
    new Student("1001001", modules[0], dictionary1),
  ...
  for (var student of students) {
    for (var mod_code of student.choices.keys()) {
      requested[students.j.course_num].append(
        (student, student.choices[mod_code])
      );
    }
  ...
  for (student in temp_students) {
    console.log(
      "swapping for " +
        student +
        " now, students who requested this dude's mods are: "
    );
    ...
    for (mod in choices.keys()) {
      ...
      for (s in current[mod.course_num]) {
        ...
        if (students.includes(s) == false) {
          ...
          console.log(
            "Oops " + s.student_id + " has already traded. remove the dude"
          );
    ...
    while (
      possible_swaps.length > 0 &&
      students.includes(possible_swaps[0][0]) == false
    ) {
